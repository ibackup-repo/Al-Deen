      ...
      <div className="max-w-3xl mx-auto w-full space-y-4">
        ...
        <Container className="!p-5 space-y-4">
          <div className="space-y-2">
            ...
            <Input type="number" min={0} value={estate} On_Change={(e) => setEstate(num(e.target.value))} />
          </div>

          <div className="grid sm:grid-cols-2 gap-4">
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={h.spouseHusband} On_Change={(e) => setH({ ...h, spouseHusband: e.target.checked, spouseWifeCount: e.target.checked ? 0 : h.spouseWifeCount })} />
            ...
            <div className="space-y-1">
              ...
              <Input type="number" min={0} max={4} value={h.spouseWifeCount} disabled={h.spouseHusband}
                On_Change={(e) => setH({ ...h, spouseWifeCount: Math.min(4, num(e.target.value)) })} />
            </div>

            <div className="space-y-1"><Label>Sons</Label>
              <Input type="number" min={0} value={h.sons} On_Change={(e) => setH({ ...h, sons: num(e.target.value) })} />
            </div>
            <div className="space-y-1"><Label>Daughters</Label>
              <Input type="number" min={0} value={h.daughters} On_Change={(e) => setH({ ...h, daughters: num(e.target.value) })} />
            </div>

            <label className="flex items-center gap-2">
              <input type="checkbox" checked={h.father} On_Change={(e) => setH({ ...h, father: e.target.checked })} />
              <span>Father alive</span>
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={h.mother} On_Change={(e) => setH({ ...h, mother: e.target.checked })} />
              <span>Mother alive</span>
            </label>

            <div className="space-y-1"><Label>Full brothers</Label>
              <Input type="number" min={0} value={h.fullBrothers} On_Change={(e) => setH({ ...h, fullBrothers: num(e.target.value) })} />
            </div>
            <div className="space-y-1"><Label>Full sisters</Label>
              <Input type="number" min={0} value={h.fullSisters} On_Change={(e) => setH({ ...h, fullSisters: num(e.target.value) })} />
            </div>
          </div>
        </Container>
