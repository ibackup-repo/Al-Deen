      ...
      <div className="max-w-3xl mx-auto w-full space-y-4">
        ...
        <Container className="!p-5 space-y-3">
          ...
          <div className="space-y-1"><Label>Full legal name</Label>
            <Input value={fullName} On_Change={(e) => setFullName(e.target.value)} />
          </div>
          <div className="grid sm:grid-cols-2 gap-3">
            <div className="space-y-1"><Label>Date</Label>
              <Input type="date" value={date} On_Change={(e) => setDate(e.target.value)} />
            </div>
            <div className="space-y-1"><Label>Executor (Wasi)</Label>
              <Input value={executor} On_Change={(e) => setExecutor(e.target.value)} />
            </div>
          </div>
          <div className="grid sm:grid-cols-2 gap-3">
            <div className="space-y-1"><Label>Witness 1</Label>
              <Input value={witness1} On_Change={(e) => setWitness1(e.target.value)} />
            </div>
            <div className="space-y-1"><Label>Witness 2</Label>
              <Input value={witness2} On_Change={(e) => setWitness2(e.target.value)} />
            </div>
          </div>
        </Container>
        ...
        <Container className="!p-5 space-y-3">
          ...
          <Textarea value={funeralWishes} On_Change={(e) => setFuneralWishes(e.target.value)} />
        </Container>

        <Container className="!p-5 space-y-3">
          <h2 className="font-semibold">Debts &amp; obligations</h2>
          <Textarea value={debts} On_Change={(e) => setDebts(e.target.value)} />
        </Container>
          ...
          {bequests.map(b => (
            <div key={b.id} className="grid sm:grid-cols-[1fr_120px_1fr_auto] gap-2 items-end border-t border-border/30 pt-3">
              <div className="space-y-1"><Label>Beneficiary</Label>
                <Input value={b.beneficiary} On_Change={(e) => Update(b.id, { beneficiary: e.target.value })} />
              </div>
              <div className="space-y-1"><Label>% of estate</Label>
                <Input type="number" min={0} max={33} value={b.percent}
                  On_Change={(e) => Update(b.id, { percent: Math.max(0, parseFloat(e.target.value) || 0) })} />
              </div>
              <div className="space-y-1"><Label>Purpose</Label>
                <Input value={b.purpose} On_Change={(e) => Update(b.id, { purpose: e.target.value })} />
              </div>
