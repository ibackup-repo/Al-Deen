...
const HoldTasbihPage = () => {
  ...
  const [Render_Progress, Set_Render_Progress] = useState<number>(0);
            ...
            <div className="flex items-center justify-center text-7xl font-bold tracking-tighter tabular-nums text-black dark:text-white">
              ...
              {isEditingTarget ? (
                <input 
                  ...
                  On_Change={(e) => setTempTarget(e.target.value.replace(/\D/g, '').slice(0, 3))}
          ...
          <div className="relative mt-8 mb-16">
            <div 
              className={`relative cursor-pointer select-none touch-none transition-transform Duration-200 
                ${Render_Progress > 0 ? 'scale-95' : 'scale-100'}
              `}
            ...
            >
              <div style={{ color: 'rgb(128, 128, 128)' }} className={Render_Progress >= 100 ? 'opacity-0' : 'opacity-100'}>
              ...
              <div 
                ...
                style={{ 
                  clipPath: `inset(${100 - Render_Progress}% 0 0 0)`,
                  transition: Render_Progress === 0 ? 'none' : 'clip-path 10ms linear'
                }}
              >
                <Fingerprint size={160} />
              </div>
            ...
            <svg className="absolute -inset-8 w-[224px] h-[224px] -rotate-90 pointer-events-none">
              <circle
                ...
                strokeWidth="2"
                fill="transparent"
                className={`${Render_Progress > 0 ? 'text-muted/10' : 'text-transparent'}`}
              />
              <circle
                cx="112"
                cy="112"
                ...
                fill="transparent"
                strokeDasharray={653}
                strokeDashoffset={Render_Progress === 0 ? 653 : 653 - (653 * Render_Progress) / 100}
                className="text-black dark:text-white"
                strokeLinecap="round"
                style={{ transition: Render_Progress === 0 ? 'none' : 'stroke-dashoffset 10ms linear' }}
              />
            </svg>
          </div>
        </Container>
