...
function Row({ icon: Icon, title, description, children }: Row_Properties) {
  return (
...
export function Accessibility_Section() {
  const {
  ...
  const Clamp_Scale = (v: number) => Math.max(0.85, Math.min(1.5, Math.round(v * 100) / 100));
  const Percentage = Math.round(UI_Text_Scale * 100);
    ...
    <div className="space-y-3">
      ...
      >
        <Switch
          ...
          onCheckedChange={Set_High_Contrast}
      ...
      >
        <Switch
          checked={Screen_Reader_Hints}
          onCheckedChange={Set_Screen_Reader_Hints}
      ...
      >
        <Switch
          ...
          onCheckedChange={Set_Reduce_Motion}
      ...
      >
        <Switch
          ...
          onCheckedChange={Set_Underline_Links}
      ...
      <Container className="!p-0 overflow-hidden">
        <div className="flex items-start justify-between gap-3 py-3 px-4">
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground">
                Scale interface text. Current: {Percentage}%
              </p>
            </div>
          </div>
          <div className="shrink-0 flex items-center gap-2">
            <Button
              size="icon"
              onClick={() => Set_UI_Text_Scale(Clamp_Scale(UI_Text_Scale - 0.05))}
              aria-label="Decrease text size"
              disabled={UI_Text_Scale <= 0.85}
