    ...
    <div className="space-y-3">
      ...
      <div className="space-y-1.5">
        ...
        <Card className="py-2.5 px-4 transition-all group">
          <Select 
            value={Surah_Info_Provider} 
            onValueChange={Set_Surah_Info_Provider}
          >
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex gap-2">
          {Text_Sizes.map((size) => (
            <Card
              key={size.value}
              onClick={() => Set_Surah_Info_Text_Size(size.value)}
              className={`
                flex-1 py-2.5 px-4 text-center cursor-pointer transition-all group
                ${Surah_Info_Text_Size === size.value ? "bg-black dark:bg-white text-white dark:text-black" : ""}
              `}
            >
              <span className={`text-sm font-medium ${Surah_Info_Text_Size === size.value ? "text-white dark:text-black" : "text-foreground [.high-contrast_&]:group-hover:text-white [.high-contrast_&]:dark:group-hover:text-black"}`}>
                {size.label}
              </span>
            </Card>
          ))}
