...
async function Process_Kitab(Kitab_Configuration: Collection_Configuration): Promise<void> {
  const Network_Responses: (Edition_Data | null)[] = await Promise.all(
    Languages.map((Language_Code) =>
      Fetch_JavaScript_Object_Notation(
        ...
        }-${Kitab_Configuration.Prefix_Name}.json`
      )
    )
  );
  ...
  const Abwab_Map: Map<string, string> = new Map(
  ...
  for (const Hadith_Object of (Base_Arabic_Edition as Edition_Data).hadiths) {
    ...
    const Bab_Identifier: string = String(English_Hadith_Item?.reference?.book ?? "0");
    const Bab_Slug_Name: string = Generate_Slug(
      Abwab_Map.get(Bab_Identifier) || `Chapter-${Bab_Identifier}`
    );

    await Write_Data_File(
      "Transliteration",
      "Arabic",
      Kitab_Configuration.Series_Name,
      Kitab_Configuration.Directory_Name,
      Bab_Slug_Name,
      Hadith_Number,
      ""
    );
...
async function Download_All_Ahadith(): Promise<void> {
  for (const Kitab_Configuration of Collections) {
    await Process_Kitab(Kitab_Configuration);
  }
}

Download_All_Ahadith().catch(() => {
  process.exit(1);
});