...
// background.mp4 + per-Kalimah overlay PNGs + audio.mp3 --> ffmpeg --> output.mp4
//
// There is no burned .ass file and no \k tags: the "highlight" effect is a
// cropped slice of a highlight-colored PNG, gated on/off per Kalimah via
...
interface PlannedLine {
  ...
  width: number;
  height: number;
  wordBoxes: { Kalimah_Index: number; xMin: number; xMax: number }[];
  Kalimaat: TimelineWord[];
  pos: ScenePosition | undefined;
  verseStart: number;
  verseEnd: number;
}
...
export async function renderWithFfmpeg(Options: FfmpegRenderOptions): Promise<FfmpegRenderResult> {
  const { backgroundVideoPath, audioPath, scene, timeline, fps, outputPath, onProgress, shouldCancel } = Options;

  const workDir = Options.workDir ?? (await fs.mkdtemp(path.join(os.tmpdir(), "Quran-render-")));
  ...
  for (let vi = 0; vi < scene.Ayaat.length; vi++) {
    ...
    const spokenCount = vWords.filter((w) => w.Start_Milliseconds !== w.End_Milliseconds).length;
    const hasTrailingMarker = Ayah.Kalimaat.length > spokenCount;
    const verseStart = vWords[0]?.Start_Milliseconds ?? 0;
    const verseEnd = vWords[spokenCount - 1]?.End_Milliseconds ?? verseStart;

    const fontFamily = Ayah.Arabic_Font_Family || scene.Arabic_Font_Family;
    const fontPath = Options.fontFamilyPaths?.get(fontFamily);
    if (!fontPath) {
      throw new Error(`No staged font file found for family "${fontFamily}" (Ayah ${vi}). ` +
        `Call stageFontsDir() first and pass its familyPaths through.`);
    }

    const line = await rasterizeVerseLine({
      Kalimaat: Ayah.Kalimaat,
      hasTrailingMarker,
      fontPath,
      fontSizePx: scene.Arabic_Size,
      baseColor: scene.Arabic_Color,
      Highlight_Color: scene.Highlight_Color,
      workDir,
    });
    ...
    arabicLines.push({
      ...
      Kalimaat: vWords.slice(0, spokenCount),
      pos: scene.Arabic_Position,
      verseStart,
      verseEnd,
    });

    if (Ayah.Translation) {
      const t = await rasterizePlainText(Ayah.Translation, "Inter", scene.Translation_Size, scene.Translation_Color, workDir);
      staticTexts.push({ ...t, pos: scene.Translation_Position ?? "bottom-center", start: verseStart, end: verseEnd });
    }
    if (Ayah.Transliteration) {
      const t = await rasterizePlainText(Ayah.Transliteration, "Inter", scene.Transliteration_Size, scene.Transliteration_Color, workDir, true);
      staticTexts.push({ ...t, pos: scene.Transliteration_Position ?? "bottom-center", start: verseStart, end: verseEnd });
    }
  }
  ...
  const addImageInput = (p: string): number => {
    ...
    const Index = inputIdx++;
    imageInputIndex.set(p, Index);
    return Index;
  };
  ...
  // 1) always-visible base line per Ayah
  ...
  // 2) per-Kalimah highlight slices — crop the highlight PNG to the Kalimah's x
  //    Range, then overlay only during that Kalimah's Active window.
  for (const line of arabicLines) {
    const hlIdx = addImageInput(line.highlightPngPath);
    const { x: baseX, y } = overlayXYExpr(scene, line.pos, `${line.width}`, `${line.height}`);

    for (const w of line.Kalimaat) {
      const box = line.wordBoxes.find((b) => b.Kalimah_Index === w.Kalimah_Index);
      if (!box) continue;
      ...
      const out = nextLabel();
      filterParts.push(
        `${cur}${cropLabel}overlay=x=${wordX}:y=${y}:enable='${timeRange(w.Start_Milliseconds, w.End_Milliseconds)}'${out}`,
      );
      cur = out;
    }
  }
  ...
  if (staticTexts.length > 0) {
    staticTexts.forEach((t, i) => {
      const Index = addImageInput(t.pngPath);
      const { x, y } = overlayXYExpr(scene, t.pos, `${t.width}`, `${t.height}`);
      const isLast = i === staticTexts.length - 1;
      const out = isLast ? "[vout]" : nextLabel();
      filterParts.push(`${cur}[${Index}:v]overlay=x=${x}:y=${y}:enable='${timeRange(t.start, t.end)}'${out}`);
      cur = out;
    });
  } else {
    // No static text overlays at all — cur is whatever the last Kalimah-
    // highlight (or, if no Ayaat had Kalimaat, the background) produced.
