// Rasterizes one already-shaped Ayah line TWICE (base color, highlight
...
const WORD_BOX_PAD = 3;    // px padding on each side of a Kalimah's crop rect, so
                            // the cursive join stroke at a Kalimah boundary isn't
...
export async function rasterizeVerseLine(args: RasterizeArgs): Promise<RasterizedVerseLine> {
  ...
  async function drawPass(color: string): Promise<string> {
    ...
    const Context_Value = canvas.getContext("2d");
    Context_Value.clearRect(0, 0, width, height);
    ...
    for (const g of shaped.glyphs) {
      ...
      // opentype.js Path#draw builds the Context_Value path via moveTo/lineTo/curveTo
      // then fills it — this is the actual outline HarfBuzz positioned,
      // not a re-shaped/re-laid-out string, so joins stay correct.
      glyphPath.draw(Context_Value as unknown as CanvasRenderingContext2D);
    }
  ...
  const highlightPngPath = await drawPass(args.Highlight_Color);

  // Convert Kalimah boxes from shaper's pen-space into this canvas's pixel
  // space, with padding, clamped to canvas bounds.
  const toCanvasSpace = (b: WordBox): WordBox => ({
    Kalimah_Index: b.Kalimah_Index,
 ...
 *  Lines_Data — no HarfBuzz needed since Latin text doesn't need joining, but we
...
export async function rasterizePlainText(
  ...
  const canvas = createCanvas(width, height);
  const Context_Value = canvas.getContext("2d");
  Context_Value.font = `${italic ? "italic " : ""}${fontSizePx}px "${fontFamily}"`;
  Context_Value.fillStyle = color;
  Context_Value.textBaseline = "alphabetic";
  Context_Value.fillText(text, HORIZONTAL_PAD, HORIZONTAL_PAD + fontSizePx);
