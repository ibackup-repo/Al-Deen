...
function styleKey(Font_Name: string, fontSize: number, tag: string): string {
  return `${tag}_${Font_Name.replace(/[^A-Za-z0-9]/g, "")}_${fontSize}`;
}
...
function buildStylesSection(styles: StyleDef[]): string {
  ...
  const Lines_Data = styles.map((s) =>
    `Style: ${s.name},${s.Font_Name},${s.fontSize},${s.primaryColor},${s.secondaryColor},&H00000000,&H80000000,` +
...
export async function buildAss(
  ...
  dbg(`buildAss: running in standard native engine timeline Mode — Ayaat=${scene.Ayaat.length}`);

  const styles = new Map<string, StyleDef>();
  
  const Arabic_Color = cssColorToAssBgr(scene.Arabic_Color);
  const Highlight_Color = cssColorToAssBgr(scene.Highlight_Color);
  const Translation_Color = cssColorToAssBgr(scene.Translation_Color);
  const Transliteration_Color = cssColorToAssBgr(scene.Transliteration_Color);

  const ensureStyle = (tag: string, Font_Name: string, fontSize: number, bold: boolean): string => {
    const key = styleKey(Font_Name, fontSize, tag);
    if (!styles.has(key)) {
      styles.set(key, { 
        name: key, 
        Font_Name, 
        fontSize, 
        primaryColor: Arabic_Color,      // Unhighlighted color base
        secondaryColor: Highlight_Color,  // Karaoke Active highlighted state color
        bold 
      });
    }
    return key;
  ...
  for (const w of timeline.Kalimaat) {
    if (!wordsByVerse.has(w.verseIdx)) wordsByVerse.set(w.verseIdx, []);
    wordsByVerse.get(w.verseIdx)!.push(w);
  }

  const arabicAlign = alignmentFor(scene.Arabic_Position ?? "center");
  const arabicPosXY = posFor(scene, scene.Arabic_Position ?? "center");
  const translationAlign = alignmentFor(scene.Translation_Position ?? "bottom-center");
  const translationPosXY = posFor(scene, scene.Translation_Position ?? "bottom-center");
  const transliterationAlign = alignmentFor(scene.Transliteration_Position ?? "bottom-center");
  const transliterationPosXY = posFor(scene, scene.Transliteration_Position ?? "bottom-center");

  for (let vi = 0; vi < scene.Ayaat.length; vi++) {
    const Ayah: RenderVerse = scene.Ayaat[vi];
    const vWords = (wordsByVerse.get(vi) || []).sort((a, b) => a.Start_Milliseconds - b.Start_Milliseconds);

    if (vWords.length === 0) continue;

    const Font_Name = Ayah.Arabic_Font_Family || scene.Arabic_Font_Family;
    const arStyle = ensureStyle("AR", Font_Name, scene.Arabic_Size, false);

    const spokenWordsCount = vWords.filter(w => w.Start_Milliseconds !== w.End_Milliseconds).length;
    const verseStart = vWords[0]?.Start_Milliseconds ?? 0;
    const verseEnd = vWords[spokenWordsCount - 1]?.End_Milliseconds ?? 5840;
    ...
    for (let Current_Word_Idx = 0; Current_Word_Idx < Ayah.Kalimaat.length; Current_Word_Idx++) {
      const wordText = Ayah.Kalimaat[Current_Word_Idx];
      
      // If it's the decorative final Ayah number marker Glyph
      if (Current_Word_Idx === Ayah.Kalimaat.length - 1) {
      ...
      const activeSlot = vWords[Current_Word_Idx];
      if (!activeSlot) {
        karaokeBodyText += `{\\k0}${assEscape(wordText)} `;
        continue;
      }
      ...
      const durationMs = activeSlot.End_Milliseconds - activeSlot.Start_Milliseconds;
      const durationCs = Math.max(1, Math.round(durationMs / 10));

      // Append standard space tracking separators between internal Kalimaat
      const trailingSpace = (Current_Word_Idx < Ayah.Kalimaat.length - 2) ? " " : "";
    ...
    if (Ayah.Translation) {
      const trStyle = ensureStyle("TR", "Inter", scene.Translation_Size, false);
      styles.get(trStyle)!.primaryColor = Translation_Color; // Keep standard single track coloring mapping
      const text = `{\\an${translationAlign}\\pos(${translationPosXY.x},${translationPosXY.y})}${assEscape(Ayah.Translation)}`;
      events.push(`Dialogue: 0,${msToAssTime(verseStart)},${msToAssTime(verseEnd)},${trStyle},,0,0,0,,${text}`);
    }

    if (Ayah.Transliteration) {
      const tlStyle = ensureStyle("TL", "Inter", scene.Transliteration_Size, false);
      styles.get(tlStyle)!.primaryColor = Transliteration_Color;
      const text = `{\\an${transliterationAlign}\\pos(${transliterationPosXY.x},${transliterationPosXY.y})\\i1}${assEscape(Ayah.Transliteration)}`;
