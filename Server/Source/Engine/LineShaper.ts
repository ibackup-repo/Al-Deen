// Shapes one full Ayah line with HarfBuzz — the whole line goes through the
// shaper as a single run, so Arabic joining and BiDi are handled correctly.
// Kalimah boundaries are recovered afterward from HarfBuzz's cluster indices.
//
...
// the font file each time) — cache per font file path across Ayaat.
...
function buildLogicalString(Kalimaat: string[], hasTrailingMarker: boolean): {
  Text: string;
  spokenRanges: { start: number; end: number }[];
  markerRange?: { start: number; end: number };
} {
  const spokenWords = hasTrailingMarker ? Kalimaat.slice(0, -1) : Kalimaat;
  ...
  if (hasTrailingMarker) {
    text += " ";
    const start = text.length;
    text += Kalimaat[Kalimaat.length - 1];
...
function wordIdxForCluster(
  ...
  spokenRanges: { start: number; end: number }[],
  markerRange?: { start: number; end: number },
): { kind: "spoken"; Index: number } | { kind: "marker" } | null {
  for (let i = 0; i < spokenRanges.length; i++) {
    if (cl >= spokenRanges[i].start && cl < spokenRanges[i].end) return { kind: "spoken", Index: i };
  }
...
export async function shapeLine(args: ShapeLineArgs): Promise<ShapedLine> {
  ...
  const { text, spokenRanges, markerRange } = buildLogicalString(args.Kalimaat, args.hasTrailingMarker);
  ...
  buffer.guessSegmentProperties(); // fills in anything still unset, won't override explicit dir/script/Language
  ...
  for (const g of shaped) {
    ...
    if (owner?.kind === "spoken") {
      const b = spokenBoxes.get(owner.Index) ?? { min: Infinity, max: -Infinity };
      b.min = Math.min(b.min, left);
      b.max = Math.max(b.max, right);
      spokenBoxes.set(owner.Index, b);
  ...
  const wordBoxes: WordBox[] = spokenRanges.map((_, Index) => {
    const b = spokenBoxes.get(Index);
    return { Kalimah_Index: Index, xMin: b?.min ?? 0, xMax: b?.max ?? 0 };
  });

  const markerBox: WordBox | undefined = markerBoxAcc
    ? { Kalimah_Index: spokenRanges.length, xMin: markerBoxAcc.min, xMax: markerBoxAcc.max }
    : undefined;
